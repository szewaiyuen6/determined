...
from determined.core import DummyDistributedContext
...
def initialize_distributed_backend() -> Optional[core.DistributedContext]:
    ...
    if distributed_backend.use_torch():
        ...
        return DummyDistributedContext()
    return None
...
def torch_batch_process(
    ...
    with initialize_default_inference_context() as core_context:
        ...
        if num_replica is None:
            num_replica = total_worker
        # Get global rank
        rank = core_context.distributed.get_rank()
        if data_shard_rank is None:
        ...
        skip = 0
        need_sync = True
        if isinstance(core_context.distributed, DummyDistributedContext):
            need_sync = False
        ...
        ).get_data_loader(repeat=False, skip=skip, num_replicas=num_replica, rank=data_shard_rank)
        ...
        for batch_idx in range(skip, iterate_length):
            print(f"currently processing batch_idx {batch_idx}")
