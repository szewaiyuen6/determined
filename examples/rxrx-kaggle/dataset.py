...
def get_test_loader(data, exclude_leak=False):
    test_dataset = CellularDataset(data, "test", normalization="sample")
    return DataLoader(
        test_dataset, 24, shuffle=False, num_workers=10, worker_init_fn=worker_init_fn
    )
...
class CellularDataset(Dataset):
    ...
    def __init__(
        self, root_dir, mode, split_seed=0, cv_number=0, transform=None, normalization="global"
    ):
        """
        ...
        csv_controls = pd.read_csv(
            self.root / ("train_controls.csv" if mode in ["train", "val"] else "test_controls.csv")
        )
        if all_controls:
        ...
        self.data = list(
            filter(lambda d: not (d[-1] is not None and d[-1] >= self.treatment_classes), self.data)
        )
        if move_controls:
            self.data += controls
