...
def train_runner(core_context, latest_checkpoint, trial_id, lr):
    ...
    # Script the model and store into mlflow
    mlflow.set_tracking_uri("<MLFLOW_TRACKING_URI>")
    os.environ["MLFLOW_TRACKING_USERNAME"] = "<MLFLOW_USERNAME>"
    os.environ["MLFLOW_TRACKING_PASSWORD"] = "<MLFLOW_PASSWORD>"
    mlflow.set_experiment("/train_mnist")
    with mlflow.start_run():
        client = mlflow.MlflowClient()
        try:
            client.create_registered_model("mnist_model")
        except Exception as e:
            # Expect to error out if this model already is registered with mlflow
            print(e)
